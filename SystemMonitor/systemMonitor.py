...
def Linex_monitor(time):
    ...
    log_str += "|%s|    %.2f%%   |    %.2f%%  |     %.2f%%  |   收：%dkb/发：%dkb  |\n" \
               % (current_time, cpu_per, memory_iofo.percent, disk_info.percent, net_info.bytes_recv / 2 ** 10,
                  net_info.bytes_sent / 2 ** 10)
    log_str += "|--------------------|------------|------------|-------------|-----------------------|\n"
